...
void gamefileLoadDefaults(struct gamefile *file)
{
	...
	pakSetBitflag(GAMEFILEFLAG_P1_HEADROLL, file->flags, false);
	...
	pakSetBitflag(GAMEFILEFLAG_P1_SHOWMISSIONTIME, file->flags, true);
	pakSetBitflag(GAMEFILEFLAG_P1_PAINTBALL, file->flags, true);

	pakSetBitflag(GAMEFILEFLAG_P2_FORWARDPITCH, file->flags, true);
	pakSetBitflag(GAMEFILEFLAG_P2_AUTOAIM, file->flags, false);
	...
	pakSetBitflag(GAMEFILEFLAG_P2_HEADROLL, file->flags, false);
	...
	pakSetBitflag(GAMEFILEFLAG_P2_SHOWMISSIONTIME, file->flags, true);
	...
	for (i = 0; i < ARRAYCOUNT(file->besttimes); i++) {
		for (j = 0; j < ARRAYCOUNT(file->besttimes[0]); j++) {
			file->besttimes[i][j] = 1;
		}
	}

	for (i = 0; i < 30; i++) {
		for (j = 1; j != 5; j++) {
			challengeSetCompletedByAnyPlayerWithNumPlayers(i, j, true);
		}
	}
	...
	for (i = 0; i < ARRAYCOUNT(g_GameFile.coopcompletions); i++) {
		g_GameFile.coopcompletions[i] = 1;
	}

	for (i = 0; i < ARRAYCOUNT(g_GameFile.firingrangescores); i++) {
		g_GameFile.firingrangescores[i] = 255;
	}
...
#endif
#if FOV
			optionfov = savebufferReadBits(&buffer, 8);
#endif
...
#if FOV
		savebufferOr(&buffer, optionsGetFOV(), 8);
#endif
